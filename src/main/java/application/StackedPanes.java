...
/**
 * Demonstrates placing multiple TitledPanes in a VBox so that each pane can
 * open and close independently of every other pane (unlike an Accordion, which
 * only allows a single pane to be open at a time).
 */
public class StackedPanes extends Application {
	...
	public void start(Stage stage) {
		...
		scene.getStylesheets().add(getClass().getClassLoader().getResource("fishy-fishy.css").toExternalForm());
	...
	private VBox createStackedTitledPanes() {
		...
		stackedTitledPanes.getChildren().setAll(createTitledPane("One Fish", GREEN_FISH),
				createTitledPane("Two Fish", YELLOW_FISH, GREEN_FISH), createTitledPane("Red Fish", RED_FISH),
